...
As a user, I can connect my Strava account, automatically fetch my activities into Formpath, store canonical records locally in Postgres, and store raw provider payloads locally in MinIO.
...
4. A `GET /api/activities` endpoint returns the fetched activities as JSON
5. When the token has expired, the backend refreshes it automatically and stores the new token
6. Activities are deduplicated (fetching again does not create duplicates)
...
4. **Activity Fetch** — fetch Strava API, map canonically, store in Postgres
5. **API Endpoint** — `GET /api/activities` returns stored activities as JSON
